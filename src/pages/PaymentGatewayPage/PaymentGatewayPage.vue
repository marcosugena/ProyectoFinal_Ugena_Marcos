<template>
  <div class="containera w-100 p-5">
    <RouterLink to="/" class="fixed-left fixed-start text-white"
      >Go Back</RouterLink
    >
    ...
    <form @submit.prevent="logvalidator">
      <div class="row">
        ...
        <div class="w-100 d-flex justify-content-center fs-4">
          <p class="text-white">
            Precio Total: {{ this.$route.params.totalprice }}€
          </p>
        </div>
      </div>
    ...
    async logvalidator(e) {
      ...
      if (!campoVacioEncontrado) {
        ...
        const respuesta = await axios.post(
          this.$store.getters.backurl + "/api/createcompra",
          this.Compra
        );
        ...
        await axios.post(
          this.$store.getters.backurl + "/api/createdetailcompra",
          obj
        );
        let carr = [];
